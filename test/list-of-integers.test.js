import {makeListType, Integer} from '../src/type-factories';
import {expect} from 'chai';

describe('List of Numbers (Integer ctor)', function () {
  before(function () {
    this.List = makeListType(Integer);
  ...
  it('can be reset or extended via length accessor', function () {
    ...
    expect(this.list[3]).to.equal(0);
    expect(this.list[4]).to.equal(0);
    ...
    expect(this.list[0]).to.equal(0);
    expect(this.list[1]).to.equal(0);
    expect(this.list[2]).to.equal(0);
    expect(this.list[3]).to.equal(0);
    expect(this.list[4]).to.equal(0);
  });
